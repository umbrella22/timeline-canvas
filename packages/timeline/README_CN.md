...
- 🧭 **资源排程（1.6 新增）**: 事件/轨道可选 `businessId`，严格原子导入导出（`loadScheduleData`/`exportScheduleData`），按业务身份的增量 upsert/patch/删除，视口快照订阅，以及覆盖普通/拖动/拉伸阶段的统一 `renderEventContent` 内容入口。
- 📨 **可选异步编辑协议**（`scheduleEditing`）：候选预览、typed 校验、单次提交结算与服务器修正、未知结果权威核对、单事件编辑状态查询。

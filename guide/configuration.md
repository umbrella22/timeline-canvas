...
`snapToSeconds: true` 时，事件拖动、左右边缘调整、切割预览和时间指示器定位均按 1 秒吸附，不受缩放级别或显示刻度影响。事件之间的辅助线吸附仍优先，可以精确对齐另一个事件的边界。

关闭秒级吸附后，`snapInterval` 使用分钟单位；若配置了 `scale`，则改用自定义主刻度，并在缩放达到 `secondPrecisionZoomThreshold` 后使用细分刻度。该阈值不再限制秒级编辑精度。
...
未配置时使用核心默认文字绘制；详见[渲染机制](/timeline-canvas/plugins/plugin-development/rendering.md)。
